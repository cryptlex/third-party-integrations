...
export async function getSubscriptionProperties(
  subscription: string,
  items: any[],
  parentSubscriptionPropertiesDictionary: Record<
    string,
    subscriptionProperties
  >,
  client: CtlxClientType
) {
  const parentSubscriptionProperties =
    parentSubscriptionPropertiesDictionary[subscription];
  if (parentSubscriptionProperties) {
    return parentSubscriptionProperties;
  } else { 
    const item = items.find(
      (item: any) => item.subscription.id === subscription
    );
    if (!item) {
    ...
    const template = await getLicenseTemplate(
      client,
      item.attributes.cryptlex_license_template_id
    );
    parentSubscriptionPropertiesDictionary[subscription] = {
      subscriptionInterval:
        item?.attributes?.cryptlex_license_subscription_interval ??
        template.subscriptionInterval,
      subscriptionStartTrigger: template.subscriptionStartTrigger,
    };

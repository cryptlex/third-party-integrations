...
import { getCustomAttributes, ORDER_ID_METADATA_KEY, DRIVER_METADATA_KEY, QUANTITY_MAPPING_MODE_ALLOWED_ACTIVATIONS, QUANTITY_MAPPING_MODE_LICENSE_COUNT, SUBSCRIPTION_ID_METADATA_KEY } from "../utils/getCustomAttributes";
...
export async function handleOrderCreated(
  ...
  const parentSubscriptionPropertiesDictionary: Record<string, subscriptionProperties> = {};
  if (orderCompletedData.completed && orderCompletedData.items.length > 0) {
    ...
    let metadata: Array<{ key: string; value: string; viewPermissions: [] }> = [];
    ...
    // Check if this is an add-on (has parentSubscription)
      ...
      const parentSubscriptionProperties = await getSubscriptionProperties(parentSubscriptionId, orderCompletedData.items, parentSubscriptionPropertiesDictionary, client);
      subscriptionInterval = parentSubscriptionProperties.subscriptionInterval;
      subscriptionStartTrigger = parentSubscriptionProperties.subscriptionStartTrigger;
    }
    // Regular subscription or one-time product
      ...
      subscriptionInterval = item.attributes?.cryptlex_license_subscription_interval;
    } else {
      // One time licenses
      metadata = [
        {
    ...
    if (subscriptionInterval !== undefined && subscriptionInterval !== null) {
    ...
    if (customAttributes?.mappingsQuantity === QUANTITY_MAPPING_MODE_ALLOWED_ACTIVATIONS) {
  ...
  do {
    ...
    createdCount += 1;
  } while (createdCount < Number(item.quantity) && (!customAttributes.mappingsQuantity || customAttributes.mappingsQuantity === QUANTITY_MAPPING_MODE_LICENSE_COUNT));
  }

  await Promise.all(licenseRequests.map(async (licenseRequest) => {
